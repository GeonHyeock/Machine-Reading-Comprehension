...
class MyDataset(Dataset):
    ...
    def preprocess_function(self, raw_text, max_length=384, stride=128):
        inputs = self.tokenizer(
            f"[|system|]You are EXAONE model from LG AI Research, a helpful assistant.[|endofturn|]\n[|user|] 지문을 읽고 내 질문에 답변해줘! \n 질문 : {raw_text['question'][0].strip()} \n 지문 : ",
            f"{raw_text['context'][0].strip()} \n[|assistant|]",
    ...
    def start_end(self, text, input_ids, context_start, context_end):
        start, end = 0, 0
        answer = torch.tensor(self.tokenizer(text)["input_ids"])
